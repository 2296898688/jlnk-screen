(function () {
  ...
  function flyToFarmBounds(farmId, duration) {
    ...
    flashMarker(center.lat, center.lng);
  ...
  function refreshMap() {
    ...
    if (isPlantingDataTab()) {
      currentPlots.forEach(function(p) { addPlotParcelLayers(p, plotG); });
      /* 作物场景：每个地块显示作物图标 */
      if (sceneMode === "planting") {
    ...
    if (isPlantingDataTab()) {
      ...
      currentPlots.forEach((p) => {
        ...
        const icon = L.divIcon({
          className: "yzt-marker weather",
          html: `<div class="m-core" style="background:#00D4FF;box-shadow:0 0 10px #00D4FF"></div><span class="m-label">${w.temp}°C</span>`,
          iconSize: [70, 32], iconAnchor: [35, 16]
        });
        L.marker([p.lat + 0.04, p.lng - 0.03], { icon }).addTo(weatherG)
          .bindTooltip(`<b>${p.plotName}</b><br/>气温 ${w.temp}°C · 湿度 ${w.humidity}%<br/>墒情 ${w.soilMoist}% · 土温 ${w.soilTemp}°C`, { className: "yzt-map-tooltip" })
          .on("click", () => renderMapWeatherFloat(p));
      });

      getVisibleWarnings().forEach((warn) => {
        const p = currentPlots.find((x) => x.id === warn.plotId);
        if (!p) return;
        const icon = L.divIcon({
          className: `yzt-marker warn level-${warn.level}`,
          html: `<div class="m-core"></div><span class="m-label">⚠ ${warn.type}</span>`,
          iconSize: [90, 34], iconAnchor: [45, 17]
        });
        const marker = L.marker([p.lat + 0.06, p.lng + 0.05], { icon }).addTo(disasterG);
        marker.bindPopup(
          `<b style="color:#FF6B6B">${warn.title}</b><br/>${warn.plotName || p.plotName}<br/>${warn.text}`,
          { className: "yzt-disaster-popup" }
        );
        marker.on("click", (e) => {
          L.DomEvent.stopPropagation(e);
          focusDisaster(warn.id);
        });
      });
    }
  ...
  function initMap() {
    ...
    document.getElementById("map-zoom-in").addEventListener("click", () => gisMap.zoomIn());
    document.getElementById("map-zoom-out").addEventListener("click", () => gisMap.zoomOut());
  ...
  function showPlotInfoCard(p) {
    ...
    if (!isTong) {
      ...
      document.getElementById("pic-contract-btn").onclick = function() {
        openPDFViewer("contract-ht-" + p.id + ".pdf", "承租合同 - " + pn);
      };
    } else {
